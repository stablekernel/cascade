...
| `cascade graph` (Mermaid render, four granularities) | unit plus CLI | `internal/graph/graph_test.go`, `cmd/cascade/main_test.go` | The manifest projects to a valid Mermaid diagram for jobs, stages, env, and cross-repo views |
