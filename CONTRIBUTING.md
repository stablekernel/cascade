...
- **One trigger evaluator**: every CLI-side decision that consumes a `triggers` list (orchestrate change detection, promotion preflight, anything new) must delegate to `config.MatchTrigger`/`config.MatchAnyTrigger` rather than reimplementing glob or `!`-negation matching. The generator emits the same pattern list verbatim into the workflow `paths:` filter, so a private matcher that drifts from GitHub Actions semantics makes the CLI silently disagree with the workflows it generated. The same rule covers derived trigger lists: a build-linked deploy's promotion gate resolves its effective triggers through `config.GetTriggersForDeploy`, not by reading the deploy's own `triggers` field directly.
