...
var correctnessCensus = map[string]correctnessCoverage{
	...
	"builds[].depends_on[]":                {marker: markerFollowup, note: "build-to-build needs: edge gating is not pinned; deploy-side gating is pinned by TestGM5_DependentDeploy_JudgesEffectiveResult"},
	"builds[].optional_depends_on[]":       {marker: markerFollowup, note: "optional needs: edge gating (build side) is not pinned"},
	...
	"builds[].run_policy":                  {marker: markerFollowup, note: "run_policy alters the job if: gate; the emitted if: expression is not yet pinned"},
	...
	"deploys[].optional_depends_on[]":       {marker: markerFollowup, note: "optional needs: edge gating (deploy side) is not distinctly pinned; required gating is pinned by TestGM5_DependentDeploy_JudgesEffectiveResult"},
	...
	"deploys[].run_policy":                  {marker: markerFollowup, note: "run_policy alters the deploy if: gate; the emitted if: expression is not yet pinned"},
	...
	"environments[].role":                 {marker: markerFollowup, note: "role selects prerelease/release promotion stage; the resulting ladder ordering is not pinned"},
	...
	"external[].deploys[].optional_depends_on[]":     {marker: markerFollowup, note: "external optional needs: edge gating is not pinned"},
	...
	"git.gpg_key_id":     {marker: markerFollowup, note: "GPG key id wired into the signing setup; the emitted signing config is not pinned"},
	"git.gpg_key_secret": {marker: markerFollowup, note: "GPG private-key secret wired into the signing setup; the emitted signing config is not pinned"},
	...
	"validate.run_policy":       {marker: markerFollowup, note: "run_policy alters the validate if: gate; the emitted if: expression is not yet pinned"},
	...
	"pin_mode":         {marker: markerFollowup, note: "pin_mode (tag|sha) changes whether emitted refs are SHAs or tags; not pinned"},
	"release_trigger":  {marker: markerFollowup, note: "release_trigger (push|dispatch) changes the emitted release trigger; not pinned"},
	"reconcile.source": {marker: markerFollowup, note: "reconcile source adapter changes the emitted detector; not pinned"},
	"reconcile.commit": {marker: markerFollowup, note: "reconcile commit mode changes the emitted companion commit step; not pinned"},
}
